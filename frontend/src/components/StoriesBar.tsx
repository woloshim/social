...
export default function StoriesBar({ groups, myUserId, onOpenGroup, onAddStory }: Props) {
  ...
  return (
    <div className="flex gap-3 overflow-x-auto px-3 py-3 bg-transparent border-b border-white/5">
      <div className="flex flex-col items-center gap-1 w-16 shrink-0">
        <button
          onClick={() => fileInputRef.current?.click()}
          className="relative w-14 h-14 rounded-full border border-dashed border-ink-500 bg-ink-800/60 flex items-center justify-center text-accent-400 text-xl"
        >
          +
        </button>
        <span className="text-[10px] text-ink-500 truncate w-full text-center">Добавить</span>
        <input
          ref={fileInputRef}
          type="file"
          accept="image/*,video/*"
      ...
      {groups.map((g, i) => (
        <button key={g.author.id} onClick={() => onOpenGroup(i)} className="flex flex-col items-center gap-1 w-16 shrink-0">
          <div
            className={`w-14 h-14 rounded-full p-[2px] ${g.has_unseen ? "bg-gradient-to-tr from-gold-500 to-gold-300" : "bg-ink-600"}`}
          >
            <div className="w-full h-full rounded-full bg-ink-950 p-[2px]">
              {g.author.avatar_url ? (
                ...
                <div className="w-full h-full rounded-full bg-ink-800 flex items-center justify-center text-lg text-white">
